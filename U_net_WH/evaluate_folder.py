...
def evaluate_folder(net, dataloader, device):
    net.eval()
    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        image, mask_true, name = batch['image'], batch['mask'], batch['id']
        ...
        save_path = os.path.join(os.getcwd(), 'prediction')
        if not os.path.isdir(save_path):
            os.mkdir(save_path)
        im = Image.fromarray(mask_pred.argmax(dim=1)[0].float().cpu().numpy()*255).convert('RGB')
        save_full_path = os.path.join(save_path, name[0]+'.png')
