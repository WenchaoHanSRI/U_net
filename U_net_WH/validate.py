...
def validate(net,
    ...
    experiment.config.update(dict(batch_size=batch_size,
    ...
    mask_pred, mask_true, image, f1, precision, recall, accuracy, dice = \
        evaluate_output(netv, val_loader, device, experiment, logimage)

    experiment.log({
        'Error metrics': wandb.Table(columns=['Dice', 'f1', 'Accuracy', 'Precision', 'Recall'],
                                     data=[[dice, f1, accuracy, precision, recall]])
    })
